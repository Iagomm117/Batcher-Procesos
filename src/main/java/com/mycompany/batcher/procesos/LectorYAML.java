...
public class LectorYAML {
    ...
    if(job.getResources().getMemory().contains("MB") || job.getResources().getMemory().contains("GB")){
        ...
        System.out.println(job.getId() + " " + job.getName() + " " + job.getPriority() + " " + job.getResources().getCpu_cores() + " " + job.getResources().getMemory()+ " " + job.getWorkload().getDuration_ms() + " " +job.getState());
        jobs.add(job);
    }
    }   
